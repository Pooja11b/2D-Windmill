...
void drawWindmill1() {
    ...
    // Note: These rotations accumulate.
        glBegin(GL_POLYGON);
		glVertex2f(0,0);
		glVertex2f(0.5f, 0.1f);
		glVertex2f(1.5f,0);
...
void display()
{
	...
	glPushMatrix();
	glTranslated(-7 + 14*(frameNumber % 400) / 300.0, 0, 0);
	glScaled(0.3,0.3,1);
	drawCart();
	glPopMatrix();

    glPushMatrix();
	glTranslated(-12 + 15*(frameNumber % 500) / 300.0, 0, 0);
	glScaled(0.3,0.3,1);
	drawCart();
	glPopMatrix();
...
void display1()
{
    ...
    glPushMatrix();
	glTranslated(-10 + 13*(frameNumber % 500) / 300.0, 0, 0);
	glScaled(0.3,0.3,1);
	drawCart();
    glColor3f(1,1,1);
	drawDisk(0.4);
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("OpenGL 2D Wind Mill");
